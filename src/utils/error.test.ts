...
describe('handleError', () => {
  test('should console error and send reply', () => {
    ...
    handleError(new BotError(error, {
      reply,
      update: { update_id: 12345 },
    } as unknown as Context));

    expect(consoleError).toBeCalledTimes(1);
    expect(consoleError).toBeCalledWith('Error while handling update 12345:', error);

    expect(reply).toBeCalledTimes(1);
    expect(reply).toBeCalledWith('Что-то пошло по пизде ¯\\_(ツ)_/¯');
  });

  test('should handle send reply error', () => {
    const error = new Error('some error');
    ...
    expect(consoleError.mock.calls[0]).toEqual(['Error while handling update 12345:', error]);
