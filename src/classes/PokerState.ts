...
import { getPokerCombinations, getRandomItem, shuffleItems } from '../utils';
...
export class PokerState {
  ...
  static fromRaw(ctx: CustomContext, raw: PokerStateRaw) {
  ...
  async nextTurn() {
    ...
    if (this.players.every((player) => player.folded || player.balance === 0 || (player.turnMade && player.bet === this.topBet))) {
